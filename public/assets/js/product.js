...
/* ── asking for a quote ────────────────────────────────────────────
   Both quote links carry data-quote and an href to the homepage form. The href
   is the no-JS path and stays honest; when quote-modal.js is present the click
   is intercepted so the question is asked here, over the photo and the specs the
   person is actually looking at, rather than after a page load that throws all
   of that away.

   Delegated from the document because both links live in markup generated by
   src/pdp.js and only one of the two exists on any given page. */
document.addEventListener('click', (e) => {
  const link = e.target.closest('[data-quote]');
  if (!link) return;
  if (typeof window.openQuoteModal !== 'function') return;   // href takes over
  e.preventDefault();
