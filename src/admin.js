...
// ── the image manifest ────────────────────────────────────────────
//
// Every photo the site has, from both places it can live: the ones committed to
// public/assets/images/ and listed in the generated manifest, and the ones
// uploaded to R2 through POST /api/admin/gallery. galleryImages() merges them
// into one manifest-shaped object, so everything below — the unlisted list, the
// validation in planRowsFor() — treats the two identically.
//
// This used to read the committed manifest directly, and there was a second,
// slightly different copy of the same function in shop.js. Both now go through
// gallery.js.
const readManifest = (env) => galleryImages(env);
